...
class VirtualWall:
        ...
        for line in self.snap_lines:
            ...
            if line.orientation == LineOrientation.HORIZONTAL:
                ...
                if line.alignment == HorizontalAlignment.TOP:
                    current_ref_y = art.y + art.height
                elif line.alignment == HorizontalAlignment.CENTER:
                    current_ref_y = art.y + art.height / 2
                elif line.alignment == HorizontalAlignment.BOTTOM:
                    current_ref_y = art.y

                if current_ref_y is not None:
                    print(f"Checking snap to HORIZONTAL-{alignment_str} at target_y={target_y}")
                    if abs(current_ref_y - target_y) < threshold:
                        # Snap art.y to respect alignment
                        if line.alignment == HorizontalAlignment.TOP:
                            art.y = target_y - art.height
                        elif line.alignment == HorizontalAlignment.CENTER:
                            art.y = target_y - art.height / 2
                        elif line.alignment == HorizontalAlignment.BOTTOM:
                            art.y = target_y
                        print(f"✅ Snapped: Art {alignment_str} aligned to y={target_y}")
                ...
                if line.alignment == VerticalAlignment.LEFT:
                    ...
                    current_ref_x = art.x + art.width / 2
                elif line.alignment == VerticalAlignment.RIGHT:
                    current_ref_x = art.x + art.width

                if current_ref_x is not None:
                    print(f"Checking snap to VERTICAL-{alignment_str} at target_x={target_x}")
                    if abs(current_ref_x - target_x) < threshold:
                        if line.alignment == VerticalAlignment.LEFT:
                            art.x = target_x
                        elif line.alignment == VerticalAlignment.CENTER:
                            art.x = target_x - art.width / 2
                        elif line.alignment == VerticalAlignment.RIGHT:
                            art.x = target_x - art.width
                        print(f"✅ Snapped: Art {alignment_str} aligned to x={target_x}")
    ...
    class DraggableArt:
        ...
        def create_canvas_item(self):
            ...
            image_path = self.art_data.get("image_path")
            if image_path and os.path.isfile(image_path):
                img = Image.open(image_path)
                img = img.resize((int(x2 - x1), int(y2 - y1)), Image.ANTIALIAS)
                self.image_obj = ImageTk.PhotoImage(img)
                self.id = self.wall_ref.canvas.create_image(x1, y1, anchor="nw", image=self.image_obj)
            else:
                self.id = self.wall_ref.canvas.create_rectangle(x1, y1, x2, y2, fill="lightblue", outline="black", width=2)
                self.label_id = self.wall_ref.canvas.create_text((x1 + x2) / 2, (y1 + y2) / 2, text=self.name, fill="black", font=("Arial", 10))
